    ...
    useEffect(() => {
        if (errors.name === "" && errors.email === "" && errors.password === "") {
            axios.post('http://localhost:8081/signup', values)
                ...
                .catch(err => {
                    console.log(err);
                    alert("Error al registrar la cuenta. Intente de nuevo.");
                });
        ...
        <div className='d-flex justify-content-center aling-items-center bg-primary vh-100'>
            <div className='bg-white p-3 rounded w-25'>
                ...
                <form action="" onSubmit={handleSubmit}>
                    ...
                    <button type='submit' className='btn btn-success w-100 rounded-0'><strong>Crear Cuenta</strong></button>
                    <Link to="/" className='btn btn-default border w-100 bg-light rounded-0 text-decoration-none'><strong>Iniciar Sesion</strong></Link>
                </form>
            </div>
        </div>
    )
